...
<html>
    ...
    <body>
                ...
                <div class="form-group pt-3">
                    ...
                    <select class="form-control" name="movieId" id="movieId">
                        <?php foreach ($movies as $movie): ?>
                            <option aria-describedby="movieNameHelp" value="<?= $movie['movieID'] . "/" . $movie['movieName']; ?>"><?= $movie['movieName']; ?></option>
                        <?php endforeach; ?>
                    </select>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-control" name="cinemaId" id="cinemaId">
                            ...
                            <?php foreach ($cinemas as $cinema): ?>
                                <option aria-describedby="cinemaNameHelp" value="<?= $cinema['cinemaID'] . "/" . $cinema['cinemaName']; ?>"><?= $cinema['cinemaName']; ?></option>
                            <?php endforeach; ?>
                        </select>
        ...
        if (isset($_POST['movieId'])) {
            $movieId = explode('/', filter_input(INPUT_POST, 'movieId'))[0];
            $movieName = explode('/', filter_input(INPUT_POST, 'movieId'))[1];
            ...
            if (isset($_POST['cinemaId']) && !empty($_POST['cinemaId'])) {
                $cinemaId = explode('/', filter_input(INPUT_POST, 'cinemaId'))[0];
                $cinemaName = explode('/', filter_input(INPUT_POST, 'cinemaId'))[1];
            } else {
                $cinemaId = '';
            }
        ...
        ?>
        <div class="container pt-4"><h2>Results</h2>
            <h6>
                <?php
                if (!empty($movieName)) {
                    echo "<i class='fas fa-film'></i> " . $movieName;
                }

                if (!empty($date)) {
                    echo "<br/><i class='far fa-clock'></i> " . date('d F Y', strtotime($date));
                }

                if (!empty($cinemaName)) {
                    echo "<br/><i class='far fa-building'></i> " . $cinemaName;
                }

                if (!empty($exper)) {
                    echo "<br/><i class='fas fa-glasses'></i> " . $exper;
                }
                ?>
            </h6>
            <table style="width: 100%; text-align: center" class="table table-hover table-sm mt-4">
                <thead>
                    <tr>
