...
class BookingXML {
    ...
    private $xmlFile;
    private $userId;
    private static $noOfAdults;
    private static $noOfKids;
    ...
    public function addPaymentMethod($paymentMethod, $credentials, $totalPrice) {
        ...
        $payment->appendChild($domtree->createElement('noOfAdults', self::$noOfAdults));
        $payment->appendChild($domtree->createElement('noOfKids', self::$noOfKids));
        $domtree->save($this->xmlFile);
    }

    public function addNoOfPeople($noOfAdults, $noOfKids) {
